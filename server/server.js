...
io.on('connection', (socket) => {
    ...
    socket.on('requestroom', (protocol) => {
        ...
        rooms[roomCode] = { host: socket.id, playerCount: 1, private: false, isFull: false};
    ...
    socket.on('joinOrCreateRoom', (roomCode) => {
        let wasGivenRoomCode = true
        let isHost = false
        let isClient = false
        let notJoinedOrCreated = false
        console.log(`Passed roomcode = ${roomCode}`)
        if (roomCode.startsWith("host")) {
            roomCode = roomCode.split(":")[1]
            isHost = true
        }
        if (roomCode.startsWith("client")) {
            roomCode = roomCode.split(":")[1]
            isClient = true
        }

        if ((!roomCode || roomCode.trim() === "") && isHost) {
            wasGivenRoomCode = false
            // Room code is null or empty, generate a new one
            console.log(`Creating a new room with random key`)
            roomCode = generateRoomCode();
        } else if(isClient) {
            // Look for a non-private lobby with open spots and join that one
            for(key of Object.keys(rooms)) {
                if(!rooms[key].private && !rooms[key].isFull) {
                    roomCode = key
                    console.log(`Matchmaking success, joining room: ${key} ${rooms[key]}`)
                }
            }

            if (!roomCode || roomCode.trim() === "") {
                console.log(`No room found disconnect`)
                socket.emit('roomerror', 'No empty room exists');
                return
            }
        }
        ...
        if (rooms[roomCode] && isClient) {
            ...
            if (hostSocketId) {
                ...
                rooms[roomCode].playerCount = rooms[roomCode].playerCount+1
                if (rooms[roomCode].playerCount == maxPlayersPerRoom) {
                    rooms[roomCode].isFull = true
                }
            } else if (!isClient) {
                console.log(`No valid host found for room code: ${roomCode}`);
                rooms[roomCode].host = socket.id;
            ...
            socket.emit('roomplayers', socketIdsInRoom);
            io.to(roomCode).except(socket.id).emit('playerjoin',socket.id);
            console.log(`User ${socket.id} has joined room ${roomCode} which has host ${rooms[roomCode].host} and is private ${rooms[roomCode].private} and has players ${rooms[roomCode].playerCount}`);
        } else if (isHost){
            ...
            rooms[newRoomCode] = { host: socket.id, playerCount: 1};
            if (wasGivenRoomCode) {
                console.log("making it a private room")
                rooms[roomCode].private = true
            } else {
                rooms[roomCode].private = false
            }
            socket.join(newRoomCode);
            socket.emit('roomcreated', newRoomCode);
            console.log(`Room ${newRoomCode} has been created by ${socket.id} and is private ${rooms[newRoomCode].private} and has players ${rooms[newRoomCode].playerCount}`);
        }
    });
    ...
    for (const roomCode in rooms) {
        ...
        if (rooms[roomCode][socket.id] || rooms[roomCode].host === leavingId) {
                ...
                if (rooms[roomCode].host === socket.id) {
                    const clients = Object.keys(rooms[roomCode]).filter(id => id !== 'host' && id !== 'playerCount' && id !== 'private' && id !== 'isFull');
                    if (clients.length > 0) {
